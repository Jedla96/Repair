...
public class EmployeeDataChangeByCode
{
    public static void EmployeeChangeCodeData()
    {
        ...
        {
            connection.Open();
            Console.WriteLine("Enter the Code of the employee to edit: ");
            string selectAnswer = Console.ReadLine();

            // Find all employees with matching id
            NpgsqlCommand selectCommand =
                new NpgsqlCommand($"SELECT * FROM employees WHERE code = '{selectAnswer}'", connection);
